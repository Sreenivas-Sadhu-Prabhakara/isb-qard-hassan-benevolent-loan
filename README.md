...
| **Asset Type** | Qard Hassan Facility |
| **Control Record** | Qard Hassan Facility Fulfillment Arrangement |
...
| POST | `/v1/qard-hassan-facility-fulfillment-arrangement/initiate` | Initiate |
| GET | `/v1/qard-hassan-facility-fulfillment-arrangement` | Retrieve (list) |
| GET | `/v1/qard-hassan-facility-fulfillment-arrangement/{crId}/retrieve` | Retrieve |
| PUT | `/v1/qard-hassan-facility-fulfillment-arrangement/{crId}/update` | Update |
| PUT | `/v1/qard-hassan-facility-fulfillment-arrangement/{crId}/control` | Control — body `{"action": "suspend"\|"resume"\|"terminate"}` |
...
curl -X POST localhost:8080/v1/qard-hassan-facility-fulfillment-arrangement/initiate -H 'content-type: application/json' -d '{"note":"hello"}'
```
...
```bash
mvn -B verify
docker build -t isb/isb-qard-hassan-benevolent-loan:0.1.0 .
```

## Deploy (Helm → K8s with Cilium mesh)

```bash
helm upgrade --install isb-qard-hassan-benevolent-loan ./helm -n isb-financing
```

Exposed through the platform Gateway at path prefix `/isb-qard-hassan-benevolent-loan` (Cilium Gateway API). Mesh policy (`CiliumNetworkPolicy`) allows: gateway ingress, same-area peers, Prometheus — everything else denied.
